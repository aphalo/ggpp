...
A1: As `geom_dotplot()` does not have a `stat` formal parameter and modifies the data it is impossible to access the positions of the points! As `group` is a factor a hypothetical `stat_dotplot()` would not work either, one would need to define a new geom `geom_labelled_dotoplot` to do the labelling in the same layer.

A2: `geom_point()` could be used together with a newly defined position function. This new function, possibly named `position_dotplot()`, would displace the observations' coordinates using the same methods as `geom_dotplot()` uses to stack the dots. This function does not yet exist, but would allow a simple solution within the grammar of graphics.  
