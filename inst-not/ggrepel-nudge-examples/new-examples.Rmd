...
## `geom_dotplot` _issue #207 raised by svenbioinf_
...
A: As `geom_dotplot()` does not have a `stat` formal parameter and modifies the data it is impossible to access the positions of the points! As `group` is a factor a hypothetical `stat_dotplot()` would not work either, one would need to define a new geom `geom_labelled_dotoplot` to do the labelling in the same layer.
...
In a more crowed plot we may want to allow overlaps but improve the visibility of the text with a semitransparent fill using `geom_label_repel()`.
...
A: Rather than automating this, adding a `parameter` called `point.size.area` seems an approach consistent with 'ggplot2' and very easy to implement. Meanwhile, an equivalent manual approach is as follows.
...
ggplot(df, aes(1, b, size = b)) + 
  ...
  geom_text_repel(aes(label = round(b,2), 
                      point.size = size_from_area(b),
                      point.padding = size_from_area(b) / 30),
                  label.padding = 0,
                  box.pading = 0,
...
A: Nudging can help. In this case appying nudging by speciying the y position for each label. Here we add increasing amounts of nudging. It would be better to start afresh by groups of labels. We need to sort the row in the data according to the position along the x axis for this approach to work.
...
## Left-align text but not label boxes _issue #200 raised by z3tt_
...
A: The original issue included a much more complex solution to this problem involving in addition formatting unrelated to 'ggreprl'. The simplification is achieved by using nudging.
...
ggplot(owid,
  ...
  geom_text_repel(aes(label = label),
                  size = 3,
                  position = position_nudge_to(x = max(owid$date) + days(20)),
