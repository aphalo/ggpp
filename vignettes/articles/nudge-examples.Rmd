...
_The rendered plots can slightly differ among versions of the packages or very rarely depending on the version of R. This article was rendered on `r lubridate::today()` with 'ggpp' version `r packageVersion("ggpp")`, 'ggrepel' version  `r packageVersion("ggrepel")` and 'ggplot2' version `r packageVersion("ggplot2")`._
...
Parameter `reverse` in `position_stacknudge()` is implemented consistently with
`position_stack()` from 'ggplot2', and can be used in different layers of the
same plot.
...
ggplot(data = df, aes(x, y, group = grp)) +
  ...
  geom_label_repel(aes(label = grp),
                   position = 
                     position_stacknudge(vjust = 0.5, y = 0.4, reverse = TRUE),
                   label.size = NA)
```
