...
We can use the same code as above to retrieve the currently active theme. As expected, it reflects the changes we introduuced above.
...
```{r}
(p + geom_label(colour = "red") + labs(title = "geom_label()")) / 
  (p + geom_label_s(colour = "red") + labs(title = "geom_label_s()"))
```
...
(p + geom_label() + labs(title = "geom_label()")) / 
  (p + geom_label_s())
```
...
```{r}
update_theme(geom = element_geom(colour = "black", borderwidth = 3))
(p + geom_label(linewidth = 0) + labs(title = "geom_label()")) /
