...
function `ggplot2::annotate()` is overridden when package 'ggpextra' is
...
Package 'ggpextra' adds support for various plot annotations and
...
these new aesthetics. Package 'ggpextra' currently provides
...
package 'ggpextra'. While `position_nudge()` implements shifts along $x$ and $y$
...
position. This is currently only supported by `geom_text_linked()`, but they
...
by `geom_text_linked()` as shown in the corresponding section above.
